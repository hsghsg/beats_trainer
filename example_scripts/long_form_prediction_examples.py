...
def example_single_file():
    ...
    results = predictor.predict_on_file(audio_path=audio_file, output_path=output_file)
    ...
    try:
        ...
        if audio_path.is_file():
            ...
            if args.output:
                ...
                output_path = (
                    audio_path.parent / f"{audio_path.stem}_predictions.{args.format}"
                )
            ...
            output_dir = (
                Path(args.output) if args.output else audio_path / "predictions"
            )

            results = predictor.predict_on_directory(
                input_dir=audio_path, output_dir=output_dir
            )
