...
def example_1_second_clips():
    ...
    config = Config(
        ...
        model=ModelConfig(freeze_backbone=True),
        training=TrainingConfig(max_epochs=10, learning_rate=1e-4),
    )

    # Create trainer with fixed clip duration
    trainer = BEATsTrainer.from_directory(
        data_dir="/path/to/your/dataset", config=config
    )

    print(f"Sample rate: {config.data.sample_rate} Hz")
    print(f"Clip duration: {config.data.clip_duration} seconds")
    print(
        f"Target samples per clip: {int(config.data.clip_duration * config.data.sample_rate)}"
    )
...
def example_short_clips():
    ...
    config = Config(
        ...
        training=TrainingConfig(max_epochs=20),
    )
    ...
    print(
        f"Using {config.data.clip_duration}s clips ({int(config.data.clip_duration * 16000)} samples)"
    )
...
def example_long_clips():
    ...
    config = Config(
        ...
        training=TrainingConfig(max_epochs=15),
    )
