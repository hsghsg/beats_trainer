...
def test_clip_duration_padding():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset = AudioDataset(
            dataframe=df, data_dir=temp_dir, sample_rate=16000, clip_duration=1.0
        )
        ...
        for i in range(len(dataset)):
            ...
            # Check audio length
            assert audio.shape[0] == target_samples, (
                f"Audio {i} should be {target_samples} samples"
            )

            # Check padding mask
            assert padding_mask.shape[0] == target_samples, (
                f"Padding mask {i} should be {target_samples} samples"
            )
            ...
            assert real_samples == expected_real_samples, (
                f"Audio {i}: Wrong number of real samples"
            )
            assert padded_samples == expected_padded_samples, (
                f"Audio {i}: Wrong number of padded samples"
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(len(dataset)):
            ...
            assert padding_mask.sum().item() == 0, (
                f"Audio {i}: No samples should be marked as padded"
            )
...
def test_collate_fixed_duration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert audio_batch.shape == (3, 16000), (
            "Audio batch should be (batch_size, samples)"
        )
        assert padding_batch.shape == (3, 16000), (
            "Padding batch should be (batch_size, samples)"
        )
        ...
        assert audio_batch.shape[1] == 16000, (
            "All clips should be exactly 16000 samples"
        )
...
def test_variable_vs_fixed_duration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(set(variable_lengths)) > 1, (
            "Variable dataset should have different lengths"
        )
        ...
        assert len(set(fixed_lengths)) == 1, (
            "Fixed dataset should have same length for all clips"
        )
        assert fixed_lengths[0] == 32000, (
            "Fixed clips should be 32000 samples (2 seconds at 16kHz)"
        )
