...
```powershell
.\.venv\Scripts\python.exe -u -X utf8 scripts\download_mimii_dataset.py --output-dir 'D:\dataset\MIMII' --machines fan pump --snrs -6 0 6 --workers 2 --connections 4
```
...
上图的竖线表示多个并列目录，不是实际目录名。单连接断点文件位于根目录，使用 ZIP 名加 .part 后缀；分段数据位于 .mimii_state/segments。默认每包使用 4 个连接下载 64 MiB 分段，每次 HTTP 范围请求最多传输 8 MiB，完整合并并通过官方 MD5 后改为 .zip，再清理已合并的分段。全部成功后，六个目标 ZIP 都会被删除，音频、完成清单和日志保留。
...
2. 默认使用 HTTP Range 分段续传，复用已有单连接断点及完成的分段；分段服务端响应必须与请求范围完全一致。设 --connections 1 可使用单连接模式，该模式在服务端忽略 Range 时会重新下载，避免错误拼接。
3. 网络连接失败时退避重试，默认最多连续重试 30 次；每隔约 30 秒记录下载进度与速度；分段模式按每段独立重试，遵守服务端 Retry-After；每次成功的范围请求会重置该段连续失败计数。
...
同一输出目录只运行一个脚本进程。进程内部通过 --workers 控制并发，允许 1、2、3，默认 2。--connections 控制每包连接数，允许 1、2、4、8，默认 4；同时联网的连接数最多为两者的乘积。执行期间不要手动修改正在下载、校验或解压的文件，也不要删除 .mimii_state。
...
覆盖成功后删除 ZIP、保留无关文件、重复执行跳过下载、MD5 失败保护、解压目标冲突保护、信噪比目录隔离、不安全 ZIP 路径，服务器接受或忽略 Range 的两种情况，以及分段复用、合并中断恢复和分段 MD5 失败保护。共 9 项测试（部分包含多个场景）。
