...
class LongFormPredictor:
    ...
    def __init__(
        ...
        logger.info(f"Initialized LongFormPredictor with {len(self.class_names)} classes")
        logger.info(f"Chunk length: {chunk_length}s, Overlap: {overlap}s")
    
    def load_and_preprocess_audio(self, audio_path: Union[str, Path]) -> Tuple[np.ndarray, float]:
        """
        ...
        logger.info(f"Loaded audio: {audio_path.name}, Duration: {duration:.2f}s, Sample rate: {sr}")
        
        return audio, duration
    
    def create_chunks(self, audio: np.ndarray, duration: float) -> List[Tuple[np.ndarray, float, float]]:
        """
    ...
    def predict_on_file(
        ...
        logger.info(f"Found {len(results)} detections above threshold {self.confidence_threshold}")
    ...
    def _save_as_text(self, df: pd.DataFrame, output_path: Path):
        ...
        with open(output_path, 'w') as f:
            f.write(f"Audio Prediction Results\n")
            f.write(f"=" * 50 + "\n\n")
...
def create_long_form_predictor(
    ...
    return LongFormPredictor(
        ...
        sample_rate=trainer.config.data.sample_rate if hasattr(trainer, 'config') else 16000
    )